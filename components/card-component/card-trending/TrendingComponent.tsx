...
export default function Recommendations({ type, items }: RecommendationProps) {
  return (
    <Card className="w-[341px] h-[320px] rounded-[5px]  ">
        ...
        <CardContent>
          <ul className="space-y-2">
            {items.map((item, index) => (
              <li
                key={index}
                className="flex items-start gap-2 group hover:bg-gray-100 p-2 rounded-md transition duration-200 cursor-pointer"
              >
